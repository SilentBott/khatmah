  ...
  return (
    <div
      ...
      onClick={(e) =>
        ...
        (setSelected(null), setQuickRegister(false))
      }
    >
      <div className="bg-emerald-950 w-full max-w-[420px] rounded-t-[2.5rem] sm:rounded-[2rem] border border-emerald-800 p-6 shadow-2xl animate-in fade-in slide-in-from-bottom-10 duration-300 max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-start mb-6">
          ...
          <div className="text-right">
            <h2
              ...
              className="text-2xl font-bold text-amber-400 font-serif"
            >
              {selected ? selected.name_ar : "تسجيل سريع"}
            </h2>
          </div>
        </div>
        ...
        {quickRegister && !selected && (
          <div className="mb-6 relative group text-right">
            <label className="text-[10px] text-emerald-700 font-bold uppercase mb-2 block tracking-widest">
              اختر السورة
            </label>
            <select
              className="w-full bg-emerald-900/40 border border-emerald-800 rounded-xl py-3 px-4 text-right text-lg font-bold text-amber-200 appearance-none outline-none focus:border-amber-500"
              onChange={(e) =>
                openModal(SURAHS.find((s) => s.id === parseInt(e.target.value)))
              }
            >
          ...
          <>
            {isSuraDone ? (
              <div className="py-8 flex flex-col items-center">
                ...
                <button
                  onClick={onDeleteAll}
                  className="w-full bg-red-950/40 border border-red-900/50 text-red-500 font-bold py-4 rounded-2xl flex items-center justify-center gap-2 hover:bg-red-900/60 transition-all"
                >
                  <RotateCcw className="w-5 h-5" /> إلغاء الختم / الحجز
                </button>
              </div>
            ) : (
              <>
                <div className="space-y-4 mb-8">
                  {vRanges.map((range, index) => {
                    ...
                    return (
                      <div
                        key={index}
                        className={`flex flex-row-reverse gap-2 items-center transition-all duration-300 ${!range.isActive ? "opacity-30" : "opacity-100"}`}
                      >
                ...
                <div className="flex gap-3">
                  <button
                    onClick={onDeleteAll}
                    className="w-14 bg-red-950/20 border border-red-900/30 rounded-2xl flex items-center justify-center text-red-500 hover:bg-red-900/40 transition-all"
                  >
                    <Trash2 className="w-6 h-6" />
                  </button>
                  <button
                    onClick={onClaim}
                    className="flex-1 bg-amber-500 hover:bg-amber-400 text-emerald-950 font-black py-4 rounded-2xl flex items-center justify-center gap-2 shadow-xl active:scale-95 transition-all text-base uppercase"
                  >
                    {loading ? (
